...
for label in admin hosp staff; do
  case $label in admin) U=$ADMIN ;; hosp) U=$HOSP ;; staff) U=$STAFF ;; esac
  ...
  echo "--- $label ($U) /recordings ---"
  for i in 1 2 3; do
    curl -sS -o /tmp/rec_$label -w "  Run $i: HTTP %{http_code} %{time_total}s\n" \
      -H "Authorization: Bearer $TOKEN" \
      "http://127.0.0.1:8000/api/v1/recordings?page=1&page_size=20"
  done
  python3 -c "import json; d=json.load(open('/tmp/rec_$label')); print('  total=',d.get('total'),'items=',len(d.get('items',[])))"
done
