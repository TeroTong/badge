...
def _clean_staff_id(value: str | None) -> str | None:
...
def _recording_staff_scope_condition(visible_staff_ids: set[str] | None):
    ...
    if not visible_staff_ids:
        return false()
    return Recording.staff_id.in_(visible_staff_ids)


def _recording_customer_fast_id_stmt(
    *,
    ...
    visible_visits = union_all(direct_recording_visits, linked_recording_visits).subquery()
    latest_visit_sub = (
        select(
...
async def _customer_type_map(
    ...
    conditions = [
        ...
        visit_scope_condition(scope),
        _customer_visit_order_scope_condition(scope),
    ]
...
def _visit_count_subquery(scope):
    return (
        select(Visit.customer_id, func.count(Visit.id).label("visit_count"))
        .where(visit_scope_condition(scope), _visit_has_visible_recordings_condition(scope))
        .group_by(Visit.customer_id)
        .subquery()
    )


def _won_count_subquery(scope):
    return (
        select(Visit.customer_id, func.count(Visit.id).label("closed_won_count"))
        .where(Visit.status == "closed_won", visit_scope_condition(scope), _visit_has_visible_recordings_condition(scope))
        .group_by(Visit.customer_id)
        .subquery()
    )
...
def _last_visit_subquery(scope, *, date_from: date | None = None, date_to: date | None = None):
    ...
    if date_to:
        conditions.append(Visit.visit_date <= date_to)
    return (
...
def _deposit_principal_subquery(scope):
    return (
        select(
            Visit.customer_id,
...
def _customer_has_scoped_visits(scope):
    return (
        select(Visit.id)
        .where(
            Visit.customer_id == Customer.id,
...
def _customer_has_positive_recharge(scope):
    return (
        select(Visit.id)
        .where(
            Visit.customer_id == Customer.id,
            visit_scope_condition(scope),
            _visit_has_visible_recordings_condition(scope),
            Visit.deposit_principal > 0,
        )
        .exists()
    )


async def _customer_visit_stats_map(
    db: AsyncSession,
    customer_ids: list[str],
    scope,
...
async def list_customers(
    ...
    if use_recording_fast_path:
        ...
        id_stmt = (
            ...
            .order_by(last_visit_sub.c.last_visit_at.desc().nulls_last(), Customer.created_at.desc(), Customer.id.asc())
        )
        if has_date_filter:
    ...
    if has_recordings is True and not has_date_filter and not use_recording_fast_path:
        id_stmt = id_stmt.where(scoped_recordings_exists)
    if has_recordings is False and not use_recording_fast_path:
        id_stmt = id_stmt.where(~scoped_recordings_exists)
    if has_visits is True and not use_recording_fast_path:
        id_stmt = id_stmt.where(scoped_visits_exists)
    if has_visits is False and not use_recording_fast_path:
        id_stmt = id_stmt.where(~scoped_visits_exists)
    if has_positive_recharge is True and not use_recording_fast_path:
    ...
    if use_fast_page and not include_summaries:
        page_rows_with_probe = (
            await db.execute(id_stmt.offset((page - 1) * page_size).limit(page_size + 1))
        ).all()
        has_more = len(page_rows_with_probe) > page_size
        page_rows = page_rows_with_probe[:page_size]
        total = page * page_size + 1 if has_more else (page - 1) * page_size + len(page_rows)
        date_summaries = []
    else:
        filtered_subquery = id_stmt.order_by(None).subquery()
        ...
        if include_summaries:
            ...
            date_summaries = []
        page_rows = (await db.execute(id_stmt.offset((page - 1) * page_size).limit(page_size))).all()
    customer_ids = [customer_id for customer_id, _ in page_rows]
    last_visit_by_customer_id = {customer_id: last_visit_at for customer_id, last_visit_at in page_rows}
    latest_visit_map: dict[str, datetime] = {}
    visit_stats_map: dict[str, dict[str, int | float | None]] = {}
    ...
    for customer_id in customer_ids:
        ...
        if customer is None:
            continue
        stats = visit_stats_map.get(customer_id, {})
        last_visit_at = last_visit_by_customer_id.get(customer_id)
        latest_visit_at = latest_visit_map.get(customer_id)
        results.append(
            _to_out(
                customer,
                visit_count=int(stats.get("visit_count") or 0),
                recording_count=recording_count_map.get(customer_id, 0),
                closed_won_count=int(stats.get("closed_won_count") or 0),
                total_deposit_principal=(
                    float(stats["total_deposit_principal"])
                    if stats.get("total_deposit_principal") is not None
                    else None
                ),
                customer_type=customer_type_map.get(customer_id),
                last_visit_at=(
                    latest_visit_at.isoformat()
                    if latest_visit_at
