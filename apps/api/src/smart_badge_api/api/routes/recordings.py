...
_archive_recording_list_cache_lock = asyncio.Lock()
_archive_recording_raw_list_cache: dict[str, object] = {
    "expires_at": 0.0,
    "source_key": None,
    "items": None,
}
...
def _display_date_utc_bounds(value: date) -> tuple[datetime, datetime]:
    start = datetime.combine(value, datetime.min.time(), tzinfo=_DISPLAY_TZ).astimezone(timezone.utc)
    end = datetime.combine(value + timedelta(days=1), datetime.min.time(), tzinfo=_DISPLAY_TZ).astimezone(timezone.utc)
    return start, end
...
def clear_archive_recording_list_cache() -> None:
    _archive_recording_raw_list_cache["expires_at"] = 0.0
    _archive_recording_raw_list_cache["source_key"] = None
    _archive_recording_raw_list_cache["items"] = None
...
def _load_archive_recording_raw_list_items() -> list[dict[str, object]]:
    now = time.monotonic()
    source_key = _archive_recording_list_source_key()
    cached_items = _archive_recording_raw_list_cache.get("items")
    cached_expires_at = float(_archive_recording_raw_list_cache.get("expires_at") or 0.0)
    if (
        cached_items is not None
        and cached_expires_at > now
        and _archive_recording_raw_list_cache.get("source_key") == source_key
    ):
        return deepcopy(cached_items)  # type: ignore[arg-type]

    archive_index = _load_archive_recording_index()
    items = [deepcopy(payload["summary"]) for payload in archive_index.values()]
    _archive_recording_raw_list_cache["items"] = deepcopy(items)
    _archive_recording_raw_list_cache["source_key"] = source_key
    _archive_recording_raw_list_cache["expires_at"] = now + _ARCHIVE_RECORDING_LIST_CACHE_TTL_SECONDS
    return items


async def _attach_archive_recording_bindings_for_list(
    db: AsyncSession,
    items: list[dict[str, object]],
    *,
    lightweight: bool,
) -> list[dict[str, object]]:
    target = getattr(_attach_archive_recording_bindings, "side_effect", None) or _attach_archive_recording_bindings
    try:
        signature = inspect.signature(target)
        accepts_kwargs = any(parameter.kind == inspect.Parameter.VAR_KEYWORD for parameter in signature.parameters.values())
        if "lightweight" not in signature.parameters and not accepts_kwargs:
            return await _attach_archive_recording_bindings(db, items)
    except (TypeError, ValueError):
        pass
    return await _attach_archive_recording_bindings(db, items, lightweight=lightweight)


async def _load_archive_recording_list_items(
    db: AsyncSession,
    *,
    lightweight: bool = False,
) -> list[dict[str, object]]:
    now = time.monotonic()
    source_key = (_archive_recording_list_source_key(), lightweight)
    ...
    async with _archive_recording_list_cache_lock:
        now = time.monotonic()
        cached_items = _archive_recording_list_cache.get("items")
        cached_expires_at = float(_archive_recording_list_cache.get("expires_at") or 0.0)
        if (
        ...
        items = _load_archive_recording_raw_list_items()
        items = await _attach_archive_recording_bindings_for_list(db, items, lightweight=lightweight)
        _archive_recording_list_cache["items"] = deepcopy(items)
...
def _archive_item_matches_recorded_date_range(
...
async def list_archive_recordings(
    ...
    def sort_archive_items(items_to_sort: list[dict[str, object]]) -> None:
        if normalized_sort_mode == "date_grouped_link_state":
            ...
            items_to_sort.sort(key=grouped_sort_key, reverse=True)
    ...
    if fast_page and not include_date_summaries:
        ...
        offset = 0
        while offset < len(candidates) and len(matched) <= end:
            chunk = deepcopy(candidates[offset:offset + chunk_size])
            chunk = await _attach_archive_recording_bindings_for_list(
                db,
                chunk,
                lightweight=not include_analysis_summary,
            )
            matched.extend(item for item in chunk if matches(item))
            offset += chunk_size
        ...
        total = end + 1 if len(matched) > end else len(matched)
    ...
    if date_from or date_to:
        ...
        items = await _attach_archive_recording_bindings_for_list(
            db,
            items,
            lightweight=not include_analysis_summary,
        )
    else:
        items = await _load_archive_recording_list_items(db, lightweight=not include_analysis_summary)

    filtered = [item for item in items if matches(item)]
    sort_archive_items(filtered)
...
async def list_recordings(
    ...
    if date_from:
        start_dt, _ = _display_date_utc_bounds(date_from)
...
async def push_recording_to_sap(
    ...
    if not recording:
        raise HTTPException(404, "Recording not found")
    result = await generate_sap_consultation_payloads(db, recording_id, allow_unlinked_preview=True)
...
async def batch_import_from_directory(
    ...
    for audio_file in audio_files:
        ...
        try:
            ...
            async with aiofiles.open(audio_file, "rb") as src, aiofiles.open(dest, "wb") as dst:
