...
- consumption_intent.budget 只写主客户明确表达的本次预算、可接受金额、承受上限、定金/意向金/支付金额；若客户没有直接说预算，但对某个明确报价/总价区间表现出价格压力（如反复核算、觉得贵、要求优惠、承受不了、对约2.9-3.0万元敏感），可写成“未明确；对总价约X元较敏感，倾向希望低于该区间”。员工/医生单方报价、算价、项目费用、优惠说明、效果解释（如“X块解决不了多少/改善有限”）都不能写入本次预算，可放在推荐方案细节或顾虑/成交影响因素中。若只有报价但客户没有明确预算、可承受金额或价格压力，budget 写 null。decision_factors 只写具体客观限制，不能写“治疗条件限制/时间到院限制/流程限制”这类大类。应写成“客户处于生理期，治疗时间受限”“客户外地到院不便”“支付失败影响下单”“疑似竞对同行需谨慎接待”等可读事实。价格、恢复、效果、风险、家人商量属于顾虑。
...
- staff_recommendations 只写“推荐方案”：员工/医生针对本次主客户已表达或确认的主诉，提出的明确解决方案、材料、品牌、用量、报价、疗程、安排或步骤；demand_priority 必须对应 customer_primary_demands 的 priority。recommendation/product_or_solution 只写核心方案名称或一句话结论；原文有品牌、材料、用量、部位、报价、疗程或步骤时必须拆到对应细节字段。客户单独说“多少钱/帮我算价格/几支够不够”只能作为预算、顾虑或反馈，不能当成推荐方案来源。
...
- 推荐方案和种草方案都要写完整短句，不能只写“玻尿酸填充塑形/注射改善/综合调整”。备选材料/品牌不要拆成多条，主推项放 brand/material，备选对比放 implementation_notes；若某方案既解决主诉又顺带改善其他问题，只放推荐方案。原文提到用量、点位、每边、疗程、先后顺序、禁忌、品牌或报价时，必须填入细节字段，不要全部堆进 recommendation。ASR 品牌词不确定时写“疑似XX”。customer_response 只能是“接受/犹豫/拒绝/未明确回应”。
