...
3. 客户事实只来自主客户表达/确认，或员工复述后主客户补充/追问/认可。说话人标签可能出错；即使标成“主客户/同行人”，若内容是报价、方案、医生案例、“我自己/我们员工/其他顾客”，也按员工/第三方话术处理。
...
- customer_profile 只用标签参考 category，开放值尽量取原话。客户或员工确认“以前/之前/当时做过、打过、打的、注射过、填过”的项目和材料必须进治疗历史：治疗项目写手术类/注射类/光电类，材料写具体名，如“鼻子打的贝丽菲尔/贝利菲尔/菲利菲尔/Bellafill”应提取“注射类”和“贝丽菲尔”。无医美史不能同时有具体治疗历史；负面项目必须有既往项目 + 不满意/后悔/效果差/想修复。不要输出“无/未提及/未知”等占位标签。
...
- staff_recommendations 必须是员工/医生给本次主客户的明确方案、材料、品牌、用量、报价、疗程、安排或步骤；“用X填充/注射Y改善Z”属于有效推荐，“某品牌/材料打几支/几毫升”“先A再B”也有效。方案必须服务本次主诉/适应症；主线鼻部时，不把后文顺带聊的唇部、其他客户案例或科普写成核心推荐。长录音合并时优先保留与开头主诉、医生结论、报价/成交讨论反复对应的方案。recommendation 写完整短句，不能只写“玻尿酸填充塑形/注射改善/综合调整”；原文有品牌、材料、用量、部位、报价、疗程或步骤时必须填入 recommendation 或细节字段。备选材料/品牌（如“两个选择：A、B”）不要拆成多条简单方案，应写在同一条方案的 brand 或 recommendation 中；ASR 品牌近音词需结合医美热词纠正。customer_response 只能是“接受/犹豫/拒绝/未明确回应”，demand_priority 对应主诉 priority，不对应则 []。
