...
3. 客户事实只来自客户直接表达、客户确认，或员工/医生复述后客户顺着补充/追问/认可。员工自用经历、第三方案例、泛化科普、假设话术、单方试探都不能反推为客户事实。
4. 正确率优先。面诊结果宁可漏掉弱线索，也不要输出不确定结论；除主诉/适应症的极端兜底外，所有字段都需要强证据或中等强度证据。
5. 证据等级：强=客户直接表达/确认；中=客户提出部位/项目后围绕方案、价格、恢复、风险、排期等持续互动；弱=员工/医生单方判断且客户未明确确认。默认只用强/中；只有“录音有效内容少 + 确认为医美业务场景 + 主诉或适应症为空”同时满足时，才可各兜底 1 条弱证据，并写 inference_note。
6. 当前意图优先于历史提及。既往治疗、材料、部位名本身不是本次主诉/适应症；若同一语境出现“现在仍有/融完后/残留/凹陷/不满意/想修复/想调整/继续处理/想恢复”等当前问题或处理意图，才提取当前问题。
7. 过滤否定、比较、风格边界、假设和效果范围：凡是“不是/不想/不要/如果/别人/案例/像某岁数/老了5到10岁”等表达，只能作为背景，不能当成正向事实。年龄只接受客户对年龄/出生日期问题的直接回答，或“我/本人 XX 岁”。
...
- customer_profile 只用标签参考中的 category；开放值尽量取原话。客户或员工确认“以前/之前/当时做过、打过、打的、注射过、填过”的项目和材料必须进入治疗历史标签：治疗项目写手术类/注射类/光电类等大类，历史用的设备/原材料名称写具体材料或设备名，例如“鼻子当时打的贝丽菲尔/贝利菲尔/菲利菲尔/Bellafill”应提取“治疗项目=注射类”和“历史用的设备/原材料名称=贝丽菲尔”。无医美史不能同时有具体治疗历史；负面项目必须有既往项目 + 不满意/后悔/效果差/想修复等线索。
- consumption_intent.budget 只写本次方案预算、报价、可接受金额、定金/意向金/支付金额；decision_factors 写客观限制，如生理期、孕哺、健康禁忌、特殊身份、流程/支付/到院限制。价格、恢复、效果、风险、家人商量属于顾虑。
- staff_recommendations 必须是员工/医生给客户的明确方案、材料、剂量、报价、安排或处理步骤；“用X填充/注射Y改善Z”属于有效推荐。recommendation 用简短方案名，customer_response 只能是“接受/犹豫/拒绝/未明确回应”，demand_priority 对应主诉 priority，不对应则 []。
- deal_outcome 只有“客户接受 + 付款/定金/下单/锁档/确定日期/安排治疗”等落地动作才是已成交；回去考虑/商量/对比/先不做且无落地动作为未成交。
