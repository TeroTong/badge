...
3. 按“话语功能”判断角色，不要盲信 current_role、speaker_label、工牌本人或本地启发式标签。
   工牌录音可能把离工牌更近的医生误标成“工牌本人”，也可能把真实工牌佩戴者/咨询师和客户混到同一个客户侧 speaker。
...
6. ASR 术语纠错范围。
   ...
   典型例子：注射语境中的“一字光波/一次光波/一支光波”可能是“一支玻尿酸”；“鲁板/鲁班”可能是“濡白天使”；轮廓语境中的“下划线”可能是“下颌线”；鼻基底/内侧苹果肌骨性支撑且同段强调“不含玻尿酸、不吸水、避免馒化”时，“瑞1/瑞一/瑞的一/瑞仪/瑞的仪/瑞得喜/芮得怡/read的1/为的仪”等音近词统一为“瑞德喜”。“双美”通常按胶原/胶原蛋白类材料理解，不要改成玻尿酸。
...
9. 推荐方案必须保留可执行细节：brand、material、dosage、price、course_or_frequency、
   ...
   content/recommendation 只写方案核心名称或一句话结论；品牌、材料、用量、报价、疗程、
   步骤、执行要点不要堆进同一句长文本，必须拆到对应字段。
   方案核心名称要尽量贴近顾问/医生原话，短而具体，优先写“部位 + 材料/项目 + 目的”，
   例如“中面部/内侧苹果肌先做一支瑞德喜支撑”。不要把口语方案改写成过度书面化标题，
   例如“联合抗衰提升方案”“结构支撑与提升塑形方案”“分阶段综合改善方案”；这些专业解释
   如有必要应放入 implementation_notes。
...
13. 客户标签边界要严格：
    - 既往治疗/材料/仪器标签必须有正向既往史证据，如“做过/打过/去年/上次/外院”；不要从
      ...
      证据不一定必须由客户本人亲口说出；员工/医生/陪同人员在当前面诊中明确指向当前客户的
      “刚刚做了/刚做的/之前做过/上次做过某项目”，也应作为当前客户既往史。典型例子：
      “姐姐刚刚做的热玛吉不影响”应输出近期做过热玛吉，并同步到 profile_facts。
...
3. seed_recommendations = 当前主诉之外、可选、低优先级、维养、下次/后续/转科/暂缓，
   ...
   如果某个方案在 fact_graph 里同时出现在 recommendations 和 seed_recommendations，
   但 seed 里明确写了“当前主诉之外/种草/赠送/非核心/下次/后续”，且没有报价、开单、
   支付、排期或客户明确选择本次执行证据，应只保留在 seed_recommendations。
...
7. 保留证据中的可执行细节：brand、material、dosage、price、course_or_frequency、
   ...
   等字段替代 content。
   content 保持短而清楚，只表达“做什么/解决什么”；品牌、材料、用量、报价、疗程、步骤和要点
   放入扁平字段，避免生成难读的超长方案名。
   content 要尽量贴近录音原话和顾问表达，不要为了显得专业而改写成营销式或论文式标题。
   禁用这类泛化标题：“分层/联合/分阶段抗衰提升方案”“结构支撑与提升塑形方案”
   “综合年轻化方案”。如果原文是“先打一支瑞德喜”“后面再考虑童颜针”，content 就写成
   “中面部/内侧苹果肌先做一支瑞德喜支撑”，把“先深层、后浅层、后续童颜针”等解释放入
   treatment_steps 或 implementation_notes。
...
2. 再判断“当前性”。最终适应症必须至少由以下一种证据支持：
   当前客户明确主诉、员工/医生对当前客户当前核心问题的诊断、解决当前主诉的当前推荐方案。
   种草、赠送、低优先级、下次/后续、当前主诉之外的方案不能单独支持 SAP 适应症；
   只有客户明确选择本次执行、进入报价/开单/支付/排期，或它是解决当前主诉的必要分阶段步骤时，
   才能作为最终适应症依据。单纯员工科普、客户随口提问、
...
8. 自体脂肪、脂肪胶、脂肪移植等外科填充才支持外科“面部填充”；玻尿酸、
   ...
   外科“面部填充”。眼袋手术中的眶隔脂肪释放/转移/回填泪沟属于眼袋手术
   的联合步骤，不要额外选择外科“面部填充”；除非另有额颞、太阳穴、面颊、
   苹果肌、鼻基底、全脸/整脸等独立自体脂肪/脂肪胶填充主诉或项目。
...
1. final_indications 中每条 standardized_indication 必须完全复制 candidate_indications
   中的候选字符串。
2. reason 简明说明为什么匹配；supporting_evidence 必须填直接支持的原文证据文本，
   不要只输出 D1/R2/L0011 这类内部 evidence_id 或行号。
...
4. 推荐方案和种草方案必须区分：推荐方案是针对本次主诉的当前核心方案；
   ...
   展示层方案名应是简洁结论，细节必须拆到对应字段；不要把所有细节塞进方案名里。
   展示层方案名应尽量保留顾问/医生的自然表达，不要把“打一支瑞德喜/下巴两支玻尿酸”
   改写成“分阶段综合抗衰提升方案”“结构支撑与提升塑形方案”等过度书面化术语。
...
9. 客户画像只保留客户本人的事实。不要把员工自述、医生科普、其他客户案例、产品介绍
   ...
   但员工/医生/陪同人员在当前面诊中明确指向当前客户的既往项目确认，也算客户事实，
   例如“姐姐刚刚做的热玛吉不影响”应保留为近期做过热玛吉。
...
def _agent_demand_cluster(item: dict[str, Any]) -> str:
    ...
    if any(term in text for term in ("过敏", "屏障", "修复", "泛红")) and any(
    ...
    if any(term in text for term in ("鼻小柱", "人中")) and any(
        term in text for term in ("拉出", "偏长", "缩短", "改善", "精致", "注射", "解决", "问题", "形态")
    ):
...
_AGENT_INTERNAL_EVIDENCE_ID_RE = re.compile(r"^(?:[A-Z]{1,4}\d{1,6}|L\d{3,6})$", re.IGNORECASE)


def _agent_evidence_tokens(value: object) -> list[str]:
    if isinstance(value, list):
        raw_tokens = [_clean_text(item) for item in value]
    else:
        raw_tokens = re.split(r"[\s,，;；、/|]+", _clean_text(value))
    tokens: list[str] = []
...
def _agent_evidence_is_only_internal_ids(value: object) -> bool:
    tokens = _agent_evidence_tokens(value)
    return bool(tokens) and all(_AGENT_INTERNAL_EVIDENCE_ID_RE.fullmatch(token) for token in tokens)
    ...
    for section in _EVIDENCE_LIST_SECTIONS:
        for item in _as_list(evidence_graph.get(section)):
            ...
            for key in ("id", "evidence_id", "source_id", "quote_id"):
                ...
                if item_id:
                    lookup[item_id] = text
            for key in ("line_id", "source_line_id"):
                item_id = _clean_text(item.get(key))
                if item_id:
                    lookup[item_id] = text
            for key in ("line_ids", "source_line_ids", "evidence_ids"):
                for item_id in (_clean_text(value) for value in _as_list(item.get(key))):
                    if item_id:
                        lookup[item_id] = text
    return lookup
    ...
    for key in (
        ...
        for item in _as_list(updated.get(key)):
            if not isinstance(item, dict):
                continue
            copied = dict(item)
            evidence_value = copied.get("evidence")
            ...
            if _agent_evidence_is_only_internal_ids(evidence_value):
                evidence_ids.extend(_agent_evidence_tokens(evidence_value))
            resolved = []
            for item_id in evidence_ids:
                text = lookup.get(item_id)
                if text and text not in resolved:
                    resolved.append(text)
            if resolved and (not current_evidence or _agent_evidence_is_only_internal_ids(current_evidence)):
...
def _agent_remove_rejected_indications(
    ...
    if not isinstance(indication_adjudication, dict):
        return fact_graph
    final_standardized: set[str] = set()
    final_name_body: set[tuple[str, str]] = set()
    for item in _as_list(indication_adjudication.get("final_indications")):
        if not isinstance(item, dict):
            continue
        standardized = _clean_text(item.get("standardized_indication"))
        if standardized:
            final_standardized.add(standardized)
            parts = standardized.split("|")
            if len(parts) >= 6:
                final_name_body.add((_clean_text(parts[3]), _clean_text(parts[5])))
    rejected: dict[str, str] = {}
    for item in _as_list(indication_adjudication.get("rejected_indications")):
        if not isinstance(item, dict):
            continue
        standardized = _clean_text(item.get("standardized_indication"))
        if standardized in final_standardized:
            continue
        if standardized:
    ...
    for standardized, reason in rejected.items():
        parts = standardized.split("|")
        if len(parts) >= 6:
            name_body = (_clean_text(parts[3]), _clean_text(parts[5]))
            if name_body not in final_name_body:
                rejected_name_body[name_body] = reason
...
def _agent_history_evidence_from_context(context: str) -> list[tuple[str, str]]:
    text = _clean_text(context)
    if not text:
        return []
    terms = "|".join(re.escape(term) for term in sorted(_AGENT_HISTORY_PROJECT_TYPES, key=len, reverse=True))
    patterns = (
        re.compile(
            rf"(?P<evidence>[^。！？\n]{{0,16}}(?:刚刚|刚|才|最近|之前|以前|上次|去年|今年|已经)?"
            rf"(?:做过|做了|做的|做完|刚做|刚做的|打过|打了|填过|治疗过|维养)[^。！？\n]{{0,24}}"
            rf"(?P<term>{terms})[^。！？\n]{{0,20}})"
        ),
        re.compile(
            rf"(?P<evidence>[^。！？\n]{{0,16}}(?P<term>{terms})[^。！？\n]{{0,24}}"
            rf"(?:做过|做了|做的|做完|刚做|刚做的|打过|打了|填过|治疗过|维养|不影响)[^。！？\n]{{0,16}})"
        ),
    )
    ...
    for pattern in patterns:
        for match in pattern.finditer(text):
            ...
            if not term or not evidence:
                continue
            if any(marker in evidence for marker in ("没做过", "没打过", "没有做过", "没有打过", "未做过", "未打过", "从来没")):
                continue
            key = (term, evidence)
            if key in seen:
                continue
...
def _agent_append_profile_tag_if_missing(
    ...
    *,
    ...
    category_key = _compact_key_text(category)
    value_key = _compact_key_text(value)
...
def _agent_backfill_recent_treatment_history_from_context(result: dict[str, Any], *, context: str) -> bool:
    history_matches = _agent_history_evidence_from_context(context)
    ...
    tags = [dict(item) for item in _as_list(profile.get("tags")) if isinstance(item, dict)]
    changed = False
    for term, evidence in history_matches:
        project_type = _AGENT_HISTORY_PROJECT_TYPES.get(term)
        if project_type:
...
def _agent_correct_result_brand_terms(result: dict[str, Any], *, context: str) -> bool:
    ...
    for block_name in ("staff_recommendations", "staff_seed_recommendations"):
        ...
        for item in _as_list(block.get("items")):
            ...
            canonical_brand_replacements = {
                "瑞得喜": "瑞德喜",
                "芮得怡": "瑞德喜",
            }
            if any(source in item_context for source in canonical_brand_replacements):
                for key in ("recommendation", "brand", "material", "implementation_notes", "evidence", "treatment_steps"):
                    if key in item:
                        item[key] = replace_text(item.get(key), canonical_brand_replacements)
                if _clean_text(item.get("brand")) in canonical_brand_replacements:
                    item["brand"] = canonical_brand_replacements[_clean_text(item.get("brand"))]
                    changed = True
                if _clean_text(item.get("material")) in canonical_brand_replacements:
                    item["material"] = canonical_brand_replacements[_clean_text(item.get("material"))]
                    changed = True
            ...
            rh_cues = ("read的1", "read的仪", "瑞1", "瑞一", "瑞的1", "瑞得喜", "芮得怡", "为的仪", "为的一", "五性材料")
            if nose_support_context and non_ha_context and any(term in item_context for term in rh_cues):
                ...
                for key in ("recommendation", "brand", "material", "implementation_notes", "evidence", "treatment_steps"):
                    if key in item:
                        item[key] = replace_text(item.get(key), replacements)
                if _clean_text(item.get("brand")) in {"", "瑞1", "瑞一", "瑞的1", "瑞得喜", "芮得怡", "read的1", "read的1（音似）", "为的仪", "瑞蓝1", "瑞蓝1号"}:
                    item["brand"] = "瑞德喜"
                    changed = True
                material = _clean_text(item.get("material"))
...
def _agent_result_demand_key(item: dict[str, Any]) -> str:
    ...
    if any(term in text for term in ("上眼睑", "上睑", "泡泡眼", "肿泡", "抛起来", "上眼皮")) and any(
        term in text for term in ("鼓", "泡", "抛", "肿", "脂肪", "改善", "解决")
    ):
        return "upper_eyelid_puffy"
    if any(term in text for term in ("隆胸", "胸部", "胸", "假体")) and any(
        term in text for term in ("容量", "变大", "放大", "增加", "250", "275", "cc", "CC", "罩杯")
    ):
        return "breast_augmentation_capacity"
    if any(term in text for term in ("自体脂肪", "脂肪填充", "全脸填充", "整脸填充", "面部填充")) and any(
        term in text for term in ("全脸", "整脸", "面部", "脸")
    ):
        return "face_fat_fill_overall"
    if any(
        term in text
        for term in (
            "面部存在凹陷",
...
def _agent_repair_overinferred_budget(result: dict[str, Any], *, context: str) -> bool:
    ...
    if not budget_claims_price_limit:
        return False
    evidence_context = _agent_join_text(block.get("evidence"), block.get("decision_factors"))
    if re.search(r"(约?399元?|约?100元?|一百元?|少100|少一百)", budget + evidence_context) and any(
        term in evidence_context for term in ("下次", "核销", "换", "订单", "少100", "少一百")
    ):
        block["budget"] = None
        block["evidence"] = [
            item
            for item in _as_list(block.get("evidence"))
            if not any(term in _clean_text(item) for term in ("399", "100", "一百", "少100", "少一百"))
        ]
        result["consumption_intent"] = block
        return True
...
def _agent_prune_budget_evidence_noise(result: dict[str, Any]) -> bool:
    block = result.get("consumption_intent")
    if not isinstance(block, dict):
        return False
    changed = False
    ...
    for item in _as_list(block.get("evidence")):
        ...
        if any(term in text for term in ("拍片费", "拍片费用", "44块", "44元", "四十四", "扫码支付")):
            changed = True
            continue
...
def _agent_result_items(result: dict[str, Any], section: str) -> list[dict[str, Any]]:
    block = _as_dict(result.get(section))
    return [dict(item) for item in _as_list(block.get("items")) if isinstance(item, dict)]


def _agent_primary_participant_analysis_result(result: dict[str, Any]) -> dict[str, Any] | None:
    for participant_result in _as_list(result.get("participant_analysis_results")):
        if not isinstance(participant_result, dict):
            continue
        scope = _clean_text(participant_result.get("participant_scope"))
        analysis_result = _as_dict(participant_result.get("analysis_result"))
        if scope == "primary_customer" and analysis_result:
            return analysis_result
    return None


def _agent_restore_sparse_top_level_from_primary_participant(result: dict[str, Any]) -> bool:
    """Recover primary-customer detail when final display audit over-compresses it."""

    primary = _agent_primary_participant_analysis_result(result)
    if not primary:
        return False
    changed = False

    top_demands = _agent_result_items(result, "customer_primary_demands")
    primary_demands = _agent_result_items(primary, "customer_primary_demands")
    if primary_demands and len(primary_demands) >= 2 and len(primary_demands) > len(top_demands) + 1:
        result["customer_primary_demands"] = _as_dict(primary.get("customer_primary_demands"))
        changed = True

    top_indications = _agent_result_items(result, "standardized_indications")
    primary_indications = _agent_result_items(primary, "standardized_indications")
    if not top_indications and primary_indications:
        result["standardized_indications"] = _as_dict(primary.get("standardized_indications"))
        changed = True
    ...
    if changed:
        ...
        if isinstance(debug, dict):
            debug["agent_primary_participant_display_restored"] = True
    return changed
    ...
    *,
    ...
    if allow_indication_backfill:
        ...
        explicit_surgical_face_fill = any(
            ...
            for term in ("脂肪填充", "自体脂肪填充", "脂肪胶", "脂肪移植", "自体脂肪移植")
        )
...
def _agent_restore_missing_display_demands(
    result: dict[str, Any],
    *,
    fact_graph: dict[str, Any],
) -> dict[str, Any]:
    source_demands = [dict(item) for item in _as_list(fact_graph.get("demands")) if isinstance(item, dict)]
    if not source_demands:
        return result

    updated = dict(result)
    block = updated.setdefault("customer_primary_demands", {})
    if not isinstance(block, dict):
        block = {"inference_note": None, "summary": "", "items": []}
        updated["customer_primary_demands"] = block
    items = [dict(item) for item in _as_list(block.get("items")) if isinstance(item, dict)]
    seen_keys = {_agent_result_demand_key(item) for item in items if _agent_result_demand_key(item)}
    changed = False

    for source in source_demands:
        scope = _first_text(source, "participant_scope", "customer_scope") or "primary_customer"
        if scope not in {"primary_customer", "current_customer", "main_customer"}:
            continue
        content = _first_text(source, "content", "demand", "text", "summary")
        if not content:
            continue
        handling_status = _first_text(source, "handling_status")
        if handling_status in {"observation_only", "staff_internal", "not_current"}:
            continue
        item = {
            "priority": len(items) + 1,
            "demand": content,
            "body_part": _first_text(source, "body_part", "area", "body_part_name"),
            "evidence": _agent_existing_fact_evidence_text(source),
            "participant": _first_text(source, "participant"),
    ...
    block["items"] = items
    updated["customer_primary_demands"] = block
    _agent_dedupe_result_demands(updated)
    _agent_sync_final_display_sections(updated)
    debug = updated.setdefault("staged_pipeline_debug", {})
    if isinstance(debug, dict):
        debug["agent_display_demands_restored_from_fact_graph"] = True
    return updated
...
def _agent_ensure_common_indications(fact_graph: dict[str, Any]) -> dict[str, Any]:
    ...
    micro_injection_material_context = any(
        term in recommendation_context
        for term in (
            "玻尿酸",
            "瑞德喜",
    ...
    if not explicit_surgical_face_fill and (
        ...
        changed = changed or removed
    independent_face_fill_context = any(
        term in recommendation_context
        for term in (
            "全脸",
            "整脸",
    ...
    if not independent_face_fill_context:
        before_len = len(candidates)
        candidates = [
            item
            for item in candidates
            if not (
                _clean_text(item.get("indication_name")) == "面部填充"
    ...
    hydration_repair_context = _agent_join_text(fact_graph.get("demands"), fact_graph.get("recommendations"))
    if any(term in hydration_repair_context for term in ("水光", "补水", "娃娃针", "润致", "波波针", "修复")) and any(
        term in hydration_repair_context for term in ("干燥", "缺水", "补水", "过敏", "屏障", "修复", "激光")
    ):
        changed = _agent_add_catalog_indication(
            candidates,
            name="干燥",
            body="面部",
            evidence="当前主诉或方案出现面部补水/水光/娃娃针修复治疗，按本系统字典映射为干燥-面部",
            confidence=0.78,
            force_include=True,
        ) or changed
    ...
    has_midface_structural_injection_plan = _agent_has_face_fill_support_context(recommendation_context)
    if not has_midface_structural_injection_plan:
        before_len = len(candidates)
        candidates = [
            item
            for item in candidates
            if not (
                _clean_text(item.get("indication_name")) == "塑美"
                and any(term in _clean_text(item.get("body_part_name")) for term in ("内颊", "面颊", "中面部", "苹果肌"))
            )
        ]
        changed = changed or len(candidates) != before_len
    if has_midface_structural_injection_plan and any(term in recommendation_context for term in ("内颊", "中面部", "面中部", "苹果肌", "面颊", "脸颊", "法令纹", "鼻唇沟")) and any(
        term in recommendation_context
        for term in (
            "玻尿酸",
            "瑞德喜",
    ...
    ) and micro_injection_material_context:
        changed = _agent_add_catalog_indication(
            candidates,
            name="塑美",
            body="内颊",
    ...
    if explicit_surgical_face_fill and independent_face_fill_context:
        changed = _agent_add_catalog_indication(
            candidates,
            name="面部填充",
            body="面部",
    ...
    if any(term in recommendation_context for term in ("卧蚕", "泪沟", "眼下", "眼周", "眶下")) and any(
        term in recommendation_context for term in ("玻尿酸", "胶原", "胶原蛋白", "双美", "嗨体", "福曼", "弗缦", "溶解酶", "注射")
    ):
        changed = _agent_add_catalog_indication(
            candidates,
            name="塑美",
...
def _dialogue_with_scope_filter(corrected_dialogue: str, scope_graph: dict[str, Any]) -> tuple[str, dict[str, Any]]:
    ...
    retain_ranges: list[tuple[int, int, str]] = []
    for segment in segments:
        ...
        if end < start:
            start, end = end, start
        if _scope_segment_should_ignore(segment):
            ignore_ranges.append((start, end, _clean_text(segment.get("scope_type"))))
        else:
            retain_ranges.append((start, end, _clean_text(segment.get("scope_type"))))
    if not ignore_ranges and not retain_ranges:
        return corrected_dialogue, {"removed_line_count": 0, "kept_line_count": len(corrected_dialogue.splitlines())}
    use_retain_ranges = len(retain_ranges) >= 2
    span_start = min((start for start, _end, _scope_type in [*ignore_ranges, *retain_ranges]), default=None)
    span_end = max((end for _start, end, _scope_type in [*ignore_ranges, *retain_ranges]), default=None)
    ...
    for line in corrected_dialogue.splitlines():
        ...
        if line_no is not None:
            ...
            if (
                not should_remove
                and use_retain_ranges
                and span_start is not None
                and span_end is not None
                and span_start <= line_no <= span_end
            ):
                in_retained_segment = any(start <= line_no <= end for start, end, _scope_type in retain_ranges)
                if not in_retained_segment:
                    should_remove = True
                    remove_type = "unsegmented_gap"
        if should_remove:
            removed += 1
...
def _scope_segment_ranges(scope_graph: dict[str, Any]) -> list[tuple[int, int, str]]:
    ranges: list[tuple[int, int, str]] = []
    for segment in _as_list(scope_graph.get("segments")):
        if not isinstance(segment, dict) or _scope_segment_should_ignore(segment):
            continue
        scope = _clean_text(segment.get("participant_scope") or segment.get("customer_scope"))
        if scope not in _SCOPE_PARTICIPANT_LABELS:
            continue
        start = _line_id_to_int(segment.get("start_line_id"))
        end = _line_id_to_int(segment.get("end_line_id"))
        if start is None or end is None:
...
def _scope_for_line_id(line_id: object, ranges: list[tuple[int, int, str]]) -> str:
    line_no = _line_id_to_int(line_id)
    if line_no is None:
        return ""
...
def _scope_from_turn_ids(values: object, ranges: list[tuple[int, int, str]]) -> str:
    counts: dict[str, int] = {}
    for value in _as_list(values):
        scope = _scope_for_line_id(value, ranges)
        if not scope:
            continue
...
def _set_item_participant_scope(item: dict[str, Any], scope: str) -> bool:
    scope = _clean_text(scope)
    if scope not in _SCOPE_PARTICIPANT_LABELS:
        return False
    changed = False
    if _clean_text(item.get("participant_scope") or item.get("customer_scope")) != scope:
        item["participant_scope"] = scope
        changed = True
    label = _SCOPE_PARTICIPANT_LABELS[scope]
    current_label = _clean_text(item.get("participant") or item.get("participant_label"))
    if not current_label or (scope == "other_customer" and not current_label.startswith("同行客户")):
        item["participant"] = label
        changed = True
    elif scope == "primary_customer" and current_label != label:
        item["participant"] = label
        changed = True
    elif scope == "companion_or_family" and current_label != label:
        item["participant"] = label
        changed = True
    elif scope == "staff" and current_label != label:
        item["participant"] = label
        changed = True
    return changed


def _agent_apply_scope_participants_to_evidence_graph(
    evidence_graph: dict[str, Any],
    scope_graph: dict[str, Any],
) -> dict[str, Any]:
    ranges = _scope_segment_ranges(scope_graph)
    if not ranges or not isinstance(evidence_graph, dict):
        return evidence_graph
    updated = dict(evidence_graph)
    changed = False
    for section in _BUSINESS_EVIDENCE_SECTIONS:
        items = [dict(item) for item in _as_list(updated.get(section)) if isinstance(item, dict)]
        if not items:
            continue
        repaired: list[dict[str, Any]] = []
        for item in items:
            scope = _scope_from_turn_ids(
                _as_list(item.get("evidence_turn_ids"))
                + _as_list(item.get("turn_ids"))
                + _as_list(item.get("source_turn_ids")),
                ranges,
            )
            if scope:
                changed = _set_item_participant_scope(item, scope) or changed
            repaired.append(item)
        updated[section] = repaired
    return updated if changed else evidence_graph


def _agent_evidence_scope_index(evidence_graph: dict[str, Any]) -> dict[str, str]:
    index: dict[str, str] = {}
    for section in _BUSINESS_EVIDENCE_SECTIONS:
        for item in _as_list(evidence_graph.get(section)):
            if not isinstance(item, dict):
            ...
            if scope not in _SCOPE_PARTICIPANT_LABELS:
                continue
            # ``source_evidence_id`` is local to the original chunk and can
            # repeat after chunk merge; only merged evidence ids are stable
            # enough to drive participant repair.
            for key in ("id", "source_id"):
                evidence_id = _clean_text(item.get(key))
                if evidence_id:
                    index[evidence_id] = scope
    return index


def _agent_scope_from_fact_item(
    item: dict[str, Any],
    *,
    ranges: list[tuple[int, int, str]],
    evidence_scope_index: dict[str, str],
) -> str:
    scope = _scope_from_turn_ids(
        _as_list(item.get("evidence_turn_ids"))
        + _as_list(item.get("turn_ids"))
    ...
    for key in ("evidence_ids", "source_evidence_ids", "source_ids"):
        for evidence_id in _as_list(item.get(key)):
            scope = evidence_scope_index.get(_clean_text(evidence_id))
            if scope:
                return scope
    for key in ("evidence_id", "source_evidence_id", "source_id"):
        scope = evidence_scope_index.get(_clean_text(item.get(key)))
        if scope:
            return scope
    return ""


_AGENT_FACT_PARTICIPANT_LIST_KEYS = (
    "demands",
    "doctor_diagnoses",
    "indication_candidates",
    "recommendations",
...
def _agent_infer_indication_scope_from_scoped_facts(
    item: dict[str, Any],
    fact_graph: dict[str, Any],
) -> str:
    terms = set(_agent_indication_current_support_terms(item))
    for value in (
        _clean_text(item.get("indication_name")),
        _clean_text(item.get("body_part_name")),
    ...
    if not terms:
        return ""
    counts: dict[str, int] = {}
    for section in ("demands", "doctor_diagnoses", "recommendations", "seed_recommendations"):
        for source in _as_list(fact_graph.get(section)):
            if not isinstance(source, dict):
                continue
            scope = _clean_text(source.get("participant_scope") or source.get("customer_scope"))
            if scope not in {"primary_customer", "other_customer"}:
                continue
            text = _agent_join_text(
                _agent_item_content(source),
                _first_text(source, "body_part", "body_part_name"),
                _first_text(source, "brand", "material"),
            )
            if any(term in text for term in terms):
                counts[scope] = counts.get(scope, 0) + 1
    if not counts:
        return ""
    ordered = sorted(counts.items(), key=lambda value: value[1], reverse=True)
    if len(ordered) == 1 or ordered[0][1] > ordered[1][1]:
        return ordered[0][0]
    return ""


def _agent_apply_scope_participants_to_fact_graph(
    fact_graph: dict[str, Any],
    scope_graph: dict[str, Any],
    evidence_graph: dict[str, Any],
) -> dict[str, Any]:
    ranges = _scope_segment_ranges(scope_graph)
    evidence_scope_index = _agent_evidence_scope_index(evidence_graph)
    if (not ranges and not evidence_scope_index) or not isinstance(fact_graph, dict):
        return fact_graph
    updated = dict(fact_graph)
    changed = False
    for section in _AGENT_FACT_PARTICIPANT_LIST_KEYS:
        items = [dict(item) for item in _as_list(updated.get(section)) if isinstance(item, dict)]
        if not items:
            continue
        repaired: list[dict[str, Any]] = []
        for item in items:
            scope = _agent_scope_from_fact_item(item, ranges=ranges, evidence_scope_index=evidence_scope_index)
            if not scope and section == "indication_candidates":
                scope = _agent_infer_indication_scope_from_scoped_facts(item, updated)
            if scope:
                changed = _set_item_participant_scope(item, scope) or changed
            repaired.append(item)
        updated[section] = repaired
    deal_outcome = updated.get("deal_outcome")
    if isinstance(deal_outcome, dict):
        copied = dict(deal_outcome)
        scope = _agent_scope_from_fact_item(copied, ranges=ranges, evidence_scope_index=evidence_scope_index)
        if scope:
            changed = _set_item_participant_scope(copied, scope) or changed
        deal_items = []
        deal_items_changed = False
        for item in _as_list(copied.get("deal_items")):
            if not isinstance(item, dict):
                continue
            item_copy = dict(item)
            item_scope = _agent_scope_from_fact_item(item_copy, ranges=ranges, evidence_scope_index=evidence_scope_index)
            if item_scope:
                deal_items_changed = _set_item_participant_scope(item_copy, item_scope) or deal_items_changed
            deal_items.append(item_copy)
        if deal_items_changed:
            copied["deal_items"] = deal_items
            changed = True
        updated["deal_outcome"] = copied
    indications = [dict(item) for item in _as_list(updated.get("indication_candidates")) if isinstance(item, dict)]
    if indications:
        repaired_indications: list[dict[str, Any]] = []
        for item in indications:
            scope = _agent_infer_indication_scope_from_scoped_facts(item, updated)
            if scope:
                changed = _set_item_participant_scope(item, scope) or changed
            repaired_indications.append(item)
        updated["indication_candidates"] = repaired_indications
    return updated if changed else fact_graph
...
def _agent_demote_orphan_optional_recommendations(fact_graph: dict[str, Any]) -> dict[str, Any]:
    ...
    seed_items = [dict(item) for item in _as_list(fact_graph.get("seed_recommendations")) if isinstance(item, dict)]
    optional_markers = (
        "整体方案",
        "整体设计",
        "整体帮你分析",
    ...
    def duplicate_seed_context_for(item: dict[str, Any]) -> str:
        item_signature = _agent_plan_semantic_signature(item)
        item_content_key = _compact_key_text(_agent_item_content(item))
        item_text = _agent_plan_text(item)
        for seed in seed_items:
            seed_text = _agent_plan_text(seed)
            if not any(term in seed_text for term in seed_only_markers):
                continue
            if item_signature and item_signature == _agent_plan_semantic_signature(seed):
                return seed_text
            if item_content_key and item_content_key == _compact_key_text(_agent_item_content(seed)):
                return seed_text
            if any(term in item_text and term in seed_text for term in specific_seed_match_terms):
                return seed_text
        return ""
    ...
    for item in recommendations:
        ...
        duplicate_seed_context = duplicate_seed_context_for(item)
        ...
        is_seed_duplicate_without_current_commitment = bool(duplicate_seed_context) and any(
            term in duplicate_seed_context for term in seed_only_markers
        ) and not any(term in text for term in strong_current_markers)
        should_demote = (
            not linked_demand_ids
            and body_terms
            and not has_demand_body_support
            and any(term in text for term in optional_markers)
        ) or _agent_is_question_only_addon_recommendation(item) or is_seed_duplicate_without_current_commitment
        if should_demote:
            copied = dict(item)
...
def _agent_prune_seed_only_demands(fact_graph: dict[str, Any]) -> dict[str, Any]:
    demands = [dict(item) for item in _as_list(fact_graph.get("demands")) if isinstance(item, dict)]
    if not demands:
        return fact_graph
    current_linked_ids: set[str] = set()
    seed_linked_ids: set[str] = set()
    for item in _as_list(fact_graph.get("recommendations")):
        if not isinstance(item, dict):
            continue
        current_linked_ids.update(_clean_text(value) for value in _as_list(item.get("related_demand_ids")) if _clean_text(value))
        current_linked_ids.update(_clean_text(value) for value in _as_list(item.get("demand_priority")) if _clean_text(value))
    for item in _as_list(fact_graph.get("seed_recommendations")):
        if not isinstance(item, dict):
            continue
        seed_linked_ids.update(_clean_text(value) for value in _as_list(item.get("related_demand_ids")) if _clean_text(value))
        seed_linked_ids.update(_clean_text(value) for value in _as_list(item.get("demand_priority")) if _clean_text(value))

    kept: list[dict[str, Any]] = []
    changed = False
    for item in demands:
        demand_id = _clean_text(item.get("id"))
        text = _agent_demand_text(item)
        handling_status = _first_text(item, "handling_status")
        only_seed_linked = bool(demand_id and demand_id in seed_linked_ids and demand_id not in current_linked_ids)
        try:
            confidence = float(item.get("confidence") or 0)
        except (TypeError, ValueError):
            confidence = 0
        weak_staff_observation = (
            confidence <= 0.65
            and any(term in text for term in ("你是因为", "轻度", "一点点", "偏深", "可以送", "赠送"))
            and not any(term in text for term in ("我想", "希望", "想要", "最想", "主要想", "本次想"))
        )
        if handling_status in {"referral_or_deferred", "not_current", "observation_only"} or only_seed_linked or weak_staff_observation:
            changed = True
            continue
        kept.append(item)
    if not changed:
...
def _final_result_audit_needed(
    ...
    *,
    ...
    for item in recommendation_items + seed_recommendation_items:
        title = _first_text(item, "recommendation", "content", "plan", "summary")
        detail_text = _agent_join_text(
            item.get("brand"),
            item.get("material"),
            item.get("dosage"),
            item.get("course_or_frequency"),
            item.get("treatment_steps"),
            item.get("implementation_notes"),
        )
        if not title or not detail_text:
            continue
        if len(title) > 42 or any(
            term in title
            for term in (
                "联合后期",
                "分阶段抗衰提升方案",
            ...
            reasons.append("overwritten_recommendation_titles_need_naturalization")
            break
...
_CORRECTION_CUSTOMER_SIDE_STAFF_CUES = (
    "姐姐刚刚做",
    "姐姐刚做",
    "妹妹刚刚做",
    "妹妹刚做",
...
def analyze_transcript_agent(
    ...
    *,
    ...
    evidence_graph = _agent_apply_scope_participants_to_evidence_graph(evidence_graph, scope_graph)
    ...
    fact_graph = _agent_apply_scope_participants_to_fact_graph(fact_graph, scope_graph, evidence_graph)
    ...
    fact_graph = _agent_prune_seed_only_demands(fact_graph)
    ...
    analysis_result = _agent_restore_missing_display_demands(
        analysis_result,
        fact_graph=fact_graph,
    )
