...
_CUSTOMER_SIDE_ROLES = frozenset({"客户", "主客户", "同行人", "访客"})
...
_TREATMENT_HISTORY_HINTS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ...
    ("超声炮", ("超声炮",)),
    ("射频/黄金微针", ("射频", "黄金微针", "热拉提")),
    ("手术类", ("双眼皮", "眼袋", "鼻综合", "隆鼻", "提眉", "吸脂", "拉皮", "隆胸", "鼻翼", "手术", "外科整形")),
...
def _is_customer_side_segment(segment: dict[str, Any]) -> bool:
    return (
        _normalized_segment_role(segment) in _CUSTOMER_SIDE_ROLES
        or _normalized_segment_business_role(segment) in _CUSTOMER_SIDE_ROLES
        or _normalized_segment_label(segment) in _CUSTOMER_SIDE_ROLES
    )
...
def _looks_like_third_party_experience_clause(text: str) -> bool:
    ...
    has_relation = any(
        ...
        re.search(
            r"(?:他|她)(?:当时|之前|后来|今天|说|做|打|来|有|没|没有|想|这样|这种|这个|那种|的|脸上|皮肤|身上|鼻子|眼睛|眼袋|泪沟|嘴|唇|毛孔|痘痘|疤痕|法令纹)",
            compact,
        )
    )
    if not has_relation:
    ...
    has_experience = any(hint in compact for hint in _THIRD_PARTY_FACT_EXPERIENCE_HINTS) or bool(
        re.search(
            r"(?:他|她).{0,12}(?:做|打|说|想|有|没|没有|术后|恢复|不满意|害怕|纠结|痘痘|毛孔|眼袋|泪沟|鼻|皮肤|肤质|法令纹|纹路|唇|嘴)",
            compact,
        )
    )
...
def _infer_surgical_history_cluster_evidence(segments: list[dict[str, Any]]) -> str | None:
    for index, segment in enumerate(segments):
        ...
        window_segments = [segments[candidate] for candidate in window_indexes]
        window_text = "\n".join(_clean_text(item.get("text")) for item in window_segments if _clean_text(item.get("text")))
        if not any(keyword in window_text for keyword in ("双眼皮", "眼袋", "鼻综合", "鼻翼", "提眉")):
            continue
        if not any(
            _is_customer_side_segment(item)
            and _segment_has_staff_signature(item)
...
def _looks_like_profile_tag_statement(category: str, value: str, text: str) -> bool:
    ...
    if category == "负面项目/设备/原材料":
        if value == "无":
            return _looks_like_no_negative_project_statement(normalized)
        if _keywords_only_appear_in_third_party_clauses(normalized, (value,)):
            return False
        return value in normalized and _looks_like_treatment_history_statement(normalized)
    ...
    *,
    ...
    for item in tags:
        ...
        if category == "治疗项目":
            ...
            changed = True
            continue
        if category in {"历史用的设备/原材料名称", "负面项目/设备/原材料"}:
...
def _recommendation_evidence_supports_item(
    ...
    *,
    ...
    if not (matches_primary or matches_indication or _recommendation_has_standalone_strong_support(evidence_item, evidence)):
        return False
...
def _backfill_staff_recommendations(result_dict: dict[str, Any], *, raw: dict[str, Any]) -> bool:
    ...
    if not _clean_text(recommendations.get("summary")):
        recommendations["summary"] = "；".join(
            _clean_text(item.get("recommendation")) or _clean_text(item.get("product_or_solution"))
            for item in items[:3]
            if _clean_text(item.get("recommendation")) or _clean_text(item.get("product_or_solution"))
        )
    result_dict["staff_recommendations"] = recommendations
    return True
...
6. 推荐方案按“方案路径 + 部位”去重，保留最完整证据；医生“用X填充/注射Y改善Z”也可作为明确方案。
...
10. 年龄只接受客户直接年龄/出生日期回答；效果范围、比较、假设、案例数字不得进入年龄、summary 或 SAP 素材。
...
def _compute_inference_note(raw: dict) -> str | None:
    ...
    if not segs:
        return None
    total = len(segs)
    customer_roles = {"客户", "主客户", "同行人", "customer", "primary_customer", "visitor_companion"}
    customer_count = sum(
        1
        for s in segs
        if str(s.get("role", "")).strip() in customer_roles
    )
