...
from smart_badge_api.periodic_locks import VISIT_ORDER_CONTEXT_SYNC_LOCK_ID, periodic_advisory_lock
...
def _sap_row_matches_context(
    ...
    *,
    ...
    normalized_advisors = {_clean_text(code) for code in advisor_codes}
    normalized_advisors.discard(None)
    if normalized_advisors:
        row_advisors = _sap_row_advisor_codes(row)
        if row_advisors and not row_advisors.intersection(normalized_advisors):
            return False
...
def _matches_any_recording_context(
    *,
    ...
    normalized_advisors = {_clean_text(code) for code in advisor_codes}
    normalized_advisors.discard(None)
    for ctx_date, ctx_institution, ctx_advisor in contexts:
        if ctx_date != row_date or ctx_institution != institution_code:
            continue
        if not ctx_advisor or not normalized_advisors or ctx_advisor in normalized_advisors:
            return True
    return False
...
async def _discover_recording_sync_contexts(
    ...
    *,
    ...
    if since is not None:
        stmt = stmt.where(Recording.created_at >= since)
    rows = (
...
def _business_date_utc_bounds(date_strings: set[str]) -> tuple[datetime, datetime] | None:
    parsed_dates = sorted({parsed for item in date_strings if (parsed := _coerce_iso_date(item))})
    if not parsed_dates:
        return None
    start_local = datetime.combine(parsed_dates[0], datetime.min.time(), tzinfo=_BUSINESS_TZ)
    end_local = datetime.combine(parsed_dates[-1] + timedelta(days=1), datetime.min.time(), tzinfo=_BUSINESS_TZ)
    return start_local.astimezone(timezone.utc), end_local.astimezone(timezone.utc)


async def _discover_recording_sync_contexts_for_sap_rows(
    db: AsyncSession,
    rows: list[SapHanaVisitOrder],
) -> set[tuple[str, str, str]]:
    date_strings = {
        normalized
        for row in rows
        if (normalized := _normalize_sap_date_token(row.crtdt))
    }
    institution_codes = {
        code
        for row in rows
        if (code := _clean_text(row.jgbm))
    }
    advisor_codes = {
        code
        for row in rows
        for code in _sap_row_advisor_codes(row)
        if code
    }
    bounds = _business_date_utc_bounds(date_strings)
    if not date_strings or not institution_codes or bounds is None:
        return set()
    ...
    stmt = (
        ...
        .join(Staff, Staff.id == Recording.staff_id)
        .where(
            Recording.created_at >= start_utc,
            Recording.created_at < end_utc,
            Staff.hospital_code.in_(sorted(institution_codes)),
            Staff.hospital_code.is_not(None),
        )
    )
    if advisor_codes:
        stmt = stmt.where(Staff.external_account.in_(sorted(advisor_codes)))

    contexts: set[tuple[str, str, str]] = set()
    for created_at, external_account, hospital_code in (await db.execute(stmt)).all():
        recording_date = _business_date_from_datetime(created_at)
        if not recording_date or recording_date not in date_strings:
            continue
        contexts.add((recording_date, str(hospital_code).strip(), str(external_account or "").strip()))
    return contexts
...
async def sync_pushed_sap_hana_visit_orders_for_recording_contexts(
    ...
    *,
    ...
    """Materialize SAP-pushed visit orders immediately.

    SAP push delivery should make visit orders available for manual recording
    binding. Recording context is useful for auto-matching, but it must not
    block the order from being written to ``visit_orders``.
    """
    normalized_keys = sorted(
        {
            (str(jgbm or "").strip(), str(dzdh or "").strip())
    ...
    matched_dates = sorted(
        {
            row_date
            for row in sap_rows
            if (row_date := _normalize_sap_date_token(row.crtdt))
        }
    )
...
async def sync_visit_orders(
    ...
    *,
    ...
    contexts = await _discover_recording_sync_contexts(db, since=since)
    if not contexts:
        return VisitOrderSyncResult(synced_count=0, new_count=0, updated_count=0, date_range="")
...
async def periodic_visit_order_context_sync(
    ...
    *,
    ...
    while not stop_event.is_set():
        try:
            async with periodic_advisory_lock("visit_order_context_sync", VISIT_ORDER_CONTEXT_SYNC_LOCK_ID) as acquired:
                if acquired:
                    async with _session_factory() as db:
                        result = await retry_visit_order_sync(
                            lambda: sync_visit_orders(db, lookback_days=lookback_days),
