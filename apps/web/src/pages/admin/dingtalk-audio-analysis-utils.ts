    ...
    recommended_plan: {
      ...
      items: asArray(plan?.items)
        ...
        .map((item) => ({
          ...
          course_or_frequency: asText(item.course_or_frequency),
          treatment_steps: asArray(item.treatment_steps).map((entry) => asText(entry)).filter((entry): entry is string => Boolean(entry)),
          implementation_notes: asText(item.implementation_notes),
        })),
    },
    seed_plan: {
